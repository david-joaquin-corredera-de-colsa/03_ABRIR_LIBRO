Attribute VB_Name = "Modulo_Navegacion_y_Limpieza"
' =============================================================================
...
Public Function F011_Limpieza_Hojas_Historicas() As Integer
    ...
    ' 2. Recorrer todas las hojas del libro
    ' 3. Para cada hoja aplicar reglas de limpieza especificas
    ' 4. Gestionar hojas Import_Envio_ con logica de ordenamiento lexicografico
    ' 5. Mantener solo las N hojas mas recientes de Import_Envio_
    ' 6. Retornar codigo de resultado
    ...
    ' Paso 4: Segunda pasada - aplicar reglas de limpieza
    vLineaError = 50
    For vContadorHojas = vTotalHojas To 1 Step -1
        vNombreHoja = ThisWorkbook.Worksheets(vContadorHojas).Name
        
        ' Paso 3: Para cada hoja aplicar reglas de limpieza especificas
        vLineaError = 60
        
        ' Regla: Hojas protegidas - no hacer nada
        If fun801_Es_Hoja_Protegida(vNombreHoja) Then
            ' No hacer nada con estas hojas
            
        ' Regla: Eliminar hojas Import_Working_
        ElseIf Left(vNombreHoja, 15) = "Import_Working_" Then
            vLineaError = 70
            Call fun802_Eliminar_Hoja_Segura(vNombreHoja)
        ...
        ElseIf Left(vNombreHoja, 13) = "Import_Envio_" Then
            vLineaError = 110
            Call fun803_Gestionar_Hoja_Envio(vNombreHoja, vHojasEnvio, vNumHojasEnvio)
        End If
    Next vContadorHojas
...
' FECHA: 2025-06-03 13:54:50 UTC
' DESCRIPCION: Funcion para crear inventario de todas las hojas del libro
' PARAMETROS: Ninguno
' RETORNO: Integer (0=exito, >0=error)
' COMPATIBILIDAD: Excel 97-365, OneDrive/SharePoint/Teams
' =============================================================================
Public Function F012_Inventariar_Hojas() As Integer
    ...
    ' 2. Borrar contenido de la hoja "01_Inventario"
    ' 3. Crear encabezados en linea 2
    ' 4. Recorrer todas las hojas y recopilar informacion
    ' 5. Crear enlaces (hyperlinks) para cada hoja
    ' 6. Ordenar el listado alfabeticamente
    ' 7. Asegurar visibilidad de hoja "01_Inventario"
    ...
    vLineaError = 20
    Set vHojaInventario = fun804_Obtener_Hoja_Inventario()
    If vHojaInventario Is Nothing Then
        vResultado = 3001 ' Error: No se pudo acceder a hoja inventario
        GoTo ErrorHandler
    End If
    
    ' Paso 2: Borrar contenido de la hoja "01_Inventario"
    vLineaError = 30
    ...
    ' Formatear encabezados (compatible Excel 97)
    vLineaError = 45
    vHojaInventario.Cells(2, 2).Font.Bold = True
    vHojaInventario.Cells(2, 3).Font.Bold = True
    
    ' Paso 4: Recorrer todas las hojas y recopilar informacion
    vLineaError = 50
    For vContadorHojas = 1 To ThisWorkbook.Worksheets.Count
        vNombreHoja = ThisWorkbook.Worksheets(vContadorHojas).Name
        
        ' Escribir nombre de la hoja
        vLineaError = 60
        vHojaInventario.Cells(vFilaActual, 2).Value = vNombreHoja
        
        ' Paso 5: Crear enlaces (hyperlinks) para cada hoja
        vLineaError = 70
        Call fun805_Crear_Enlace_Hoja(vHojaInventario, vFilaActual, 3, vNombreHoja)
        
        vFilaActual = vFilaActual + 1
    Next vContadorHojas
    
    ' Paso 6: Ordenar el listado alfabeticamente (compatible Excel 97)
    vLineaError = 80
    If vFilaActual > 3 Then
        Set vRangoOrdenar = vHojaInventario.Range("B3:C" & (vFilaActual - 1))
        vRangoOrdenar.Sort Key1:=vHojaInventario.Range("B3"), Order1:=xlAscending, Header:=xlNo
    End If
    
    ' Paso 7: Asegurar visibilidad de hoja "01_Inventario"
    vLineaError = 90
    vHojaInventario.Visible = xlSheetVisible
    
    ' Ajustar columnas automaticamente
    vLineaError = 95
    vHojaInventario.Columns("B:C").AutoFit
...
' =============================================================================
' FUNCION AUXILIAR: fun801_Es_Hoja_Protegida
' FECHA: 2025-06-03 13:54:50 UTC
...
' =============================================================================
Public Function fun801_Es_Hoja_Protegida(vNombreHoja As String) As Boolean
...
' =============================================================================
' SUB AUXILIAR: fun802_Eliminar_Hoja_Segura
' FECHA: 2025-06-03 13:54:50 UTC
' DESCRIPCION: Elimina una hoja de forma segura con control de errores
' PARAMETROS: vNombreHoja (String)
' =============================================================================
Public Sub fun802_Eliminar_Hoja_Segura(vNombreHoja As String)
...
' =============================================================================
' SUB AUXILIAR: fun803_Gestionar_Hoja_Envio
' FECHA: 2025-06-03 13:54:50 UTC
...
' =============================================================================
Public Sub fun803_Gestionar_Hoja_Envio(vNombreHoja As String, vHojasEnvio() As String, vNumHojasEnvio As Integer)
...
' =============================================================================
' FUNCION AUXILIAR: fun804_Obtener_Hoja_Inventario
' FECHA: 2025-06-03 13:54:50 UTC
' DESCRIPCION: Obtiene referencia a la hoja de inventario
' RETORNO: Worksheet (objeto hoja o Nothing si error)
' =============================================================================
Public Function fun804_Obtener_Hoja_Inventario() As Worksheet
    
    On Error GoTo ErrorHandler
    
    Set fun804_Obtener_Hoja_Inventario = ThisWorkbook.Worksheets("01_Inventario")
    Exit Function
    
ErrorHandler:
    Set fun804_Obtener_Hoja_Inventario = Nothing
    
End Function

' =============================================================================
' SUB AUXILIAR: fun805_Crear_Enlace_Hoja
' FECHA: 2025-06-03 13:54:50 UTC
...
' =============================================================================
Public Sub fun805_Crear_Enlace_Hoja(vHojaDestino As Worksheet, vFila As Integer, vColumna As Integer, vNombreHoja As String)
